...
SoftwareSerial HZ1050(19, 3);                                   // Sử dụng SoftwareSerial để giao tiếp với một thiết bị khác (module có mã HZ1050)
...
String deviceId = "3";  // for test only
...
unsigned long currentMillis = 0;      // biến lưu trữ thời điểm hiện tại
const long interval = 1 * 60 * 1000;  // Thời gian giữa các lần còi kêu (5 phút * 60 giây/phút * 1000 miligiây/giây)
...
void setup(void) {
  ...
  MySerial.begin(BAUDRATE, SERIAL_8N1, RxPin, TxPin);  // Bắt đầu Serial với tốc độ baud, cấu hình dữ liệu nối tiếp và cấu hình nhất định
  ...
  // Tạo các nhiệm vụ
  //(NAME, STACK SIZE, THAM SỐ, ƯU TIÊN, LƯU TRỮ HANDLE, CORE CHẠY)
  ...
  previousMillis = millis();  // Biến lưu trữ thời gian trước đó
}
...
void battery() {
  ...
  if (voltage_value <= 2.7 && voltage_value > 0 )

  {
    // currentMillis = millis();  // Lấy thời gian hiện tại
    // Serial.print("Voltage <4.0 44444444444444444444444444444 ");
    // if (currentMillis - previousMillis >= 1000) {  // Kiểm tra xem đã đủ thời gian chưa
    // Serial.print("Voltage = fffffffffffffffffffffffff ");
    //   previousMillis = currentMillis;                  // Cập nhật thời gian trước đó
    BuzzerBat();  // Gọi hàm kêu còi
    // }
  }
...
void taskSendData2CloudFunction(void *pvParameters) {
  for (;;) {
    ...
    // Nếu giá trị value_key_13 được thay đổi thì mới update
    ...
    // if (currentMillis - lastMillis_battery >= 10UL * 1000) {  // Kiểm tra sau mỗi 10 giây
...
void setupWifi() {
  ...
  if (!wifiManager.autoConnect("BusMate")) {
